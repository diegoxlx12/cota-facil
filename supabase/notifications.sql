...
begin
  if tg_op = 'INSERT' then
    for p in select id from public.profiles where role in ('compras','gestor') loop
      insert into public.notifications(user_id, quote_id, title, message)
      values(p.id, new.id, 'Nova cotação', 'Uma nova cotação foi enviada para Compras.');
    end loop;
  elsif tg_op = 'UPDATE' and old.status is distinct from new.status then
    if new.status in ('AGUARDANDO_DECISAO','PEDIDO','VENDIDA','NAO_VENDIDA') then
      insert into public.notifications(user_id, quote_id, title, message)
      values(new.seller_id, new.id, 'Cotação atualizada',
        case new.status
          when 'AGUARDANDO_DECISAO' then 'Sua cotação foi respondida e aguarda sua decisão.'
          when 'PEDIDO' then 'Um vendedor confirmou um pedido.'
